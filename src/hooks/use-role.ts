...
export function useRole() {
  ...
  const startImpersonation = useCallback(
    async (role: Role | string) => {
      ...
      try {
        ...
        authLogger.error('Impersonation error:', error);
      } finally {
  ...
  const endImpersonation = useCallback(async () => {
    ...
    try {
      ...
      authLogger.error('Ending impersonation error:', error);
    } finally {
      setImpersonationLoading(false);
    }
  ...
  return {
    ...
    hasAnyRole: (checkRoles: (Role | string)[]) =>
      checkRoles.includes(userRole as Role | string),
    hasAllRoles: (checkRoles: (Role | string)[]) =>
      checkRoles.length === 1 && checkRoles[0] === userRole,
